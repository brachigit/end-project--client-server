...
const addNutrition=async(req,res)=>{
 ...
 const obj = await Users.findOne({name}).exec()
 
     if (obj){
       return res.status(400).json({message:'iteam already exists'})
     }
...
const deleteNutritionByID=async(req,res)=>{
    ...
    const{name,nutrientType}=req.params 
...
const SendMail=async(req,res)=>{
 ...
 module.exports={addNutrition,getNutritionByType,getNutritionByID,updateNutrition,deleteNutritionByID,SendMail}